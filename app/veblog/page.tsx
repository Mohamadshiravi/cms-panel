...
import {
  deleteVeblogsFromServer,
  getVeblogsFromServer,
} from "@/redux/slices/veblog";
...
export default function Courses() {
  ...
  async function DeleteVeblogHandler(id: string) {
    ...
    if (isOk) {
      setBtnLoading(true);
      const res = await dispatch(deleteVeblogsFromServer(id));
      if (res.payload) {
        setBtnLoading(false);
        swal({
          icon: "success",
  ...
  return (
    <>
      <section className="bg-white dark:bg-zinc-900 w-full min-h-full sm:p-6 p-3">
        <div className="flex gap-4 mt-2">
          <button
            ...
            className="bg-green-600 text-center hover:bg-green-700 dark:shadow-lg dark:shadow-green-500/20 shadow-lg shadow-green-500/50 transition w-full py-2 rounded-md text-xl text-white"
          >
            افزودن مقاله جدید
          </button>
        </div>
                ...
                <div className="flex flex-col justify-between border border-zinc-100 dark:border-zinc-800 md:rounded-l-lg md:rounded-r-none rounded-b-lg w-full h-full">
                  ...
                  <div className="flex lg:flex-row flex-col items-center justify-between lg:gap-2 gap-4 bg-zinc-100 rounded-bl-md md:rounded-br-none rounded-br-md dark:bg-zinc-800 px-2 py-2">
                    ...
                    <div className="flex gap-2 w-full justify-end">
                      <button
                        onClick={() => DeleteVeblogHandler(e._id)}
                        disabled={btnLoading}
      ...
      {isEditorOpen && <TextEditor CloseModal={CloseModal} />}
    </>
  );
  function CloseModal() {
